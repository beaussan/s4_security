...
public class ChatServer {
     ...
     */
    private final static HashSet<PrintWriter> writers = new HashSet<PrintWriter>();
    ...
    private static class Handler extends Thread {
        ...
        public void run() {
            try {
                ...
                writeMessage("Serveur","User " + name + " connected.");
                ...
                while (true) {
                    ...
                    if (input.startsWith("MESSAGE ")){
                        writeMessage(name, input.substring(8));
                    } else if (input.startsWith("COMMAND ")){
                        handleCommand(input.substring(8));
                    }
                ...
                writeMessage("Serveur","User " + name + " : " + e);
            } finally {
                // This client is going down!  Remove its name and its print
                // writer from the sets, and close its socket.
                if (name != null) {
                    names.remove(name);
                    writeMessage("Serveur","User " + name + " disconected.");
                }
                if (out != null) {
                    writers.remove(out);
                }
        ...
        private synchronized void writeMessage(String user, String message, String format){
            ...
            for (PrintWriter writer : writers) {
                writer.format(str,user,message);
            }
        }
    }
}